...
def scan_document(
    ...
    def record(name: str, segment: str, signals, subject: str) -> None:
        ...
        if _HOST_RE.match(name):
            cand.note("host")
        if not cand.evidence and segment:
            cand.evidence = segment if len(segment) <= 200 else segment[:197] + "..."

    for segment, ents in zip(read, _ner_by_segment(read, ner_fn), strict=False):
        ...
        for name, _ in gazetteer_orgs(segment):
            record(name, segment, ["gazetteer", *context], subject)
        framed = frame_named_orgs(segment)
        for name in framed:
            cleaned = clean_ner_org(name)
            if cleaned:
                record(cleaned, segment, ["naming_frame", *context], subject)
        ...
        for ent in ents:
            ...
            record(cleaned, segment, signals, subject)
        for host in _domain_orgs(segment, first_party):
            record(host, segment, ["host", *context], subject)
...
def _record_tables(doc: Document, role: str, first_party, record) -> None:
    ...
    for table in doc.tables:
        for cell in table.name_cells:
            ...
            if not name:
                continue
            record(name, cell, ["vendor_column"], subject)
